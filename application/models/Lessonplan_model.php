...
class Lessonplan_model extends MY_model
{
    ...
    public function getlessonBysessionid($session)
    {
        return $this->db->select('*')->from('lesson')->where('session_id', $session)->get()->result_array();
    }
    ...
    public function getTopicBySession($session)
    {
        return $this->db->select('*')->from('topic')->where('session_id', $session)->get()->result_array();
    }
