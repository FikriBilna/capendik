...
class Subjecttimetable_model extends MY_Model {
    ...
    public function getSyllabussubject($staff_id, $day_value, $condition) {
        $sql = "SELECT `classes`.`class`,`sections`.`section`,`subject_group_subjects`.`subject_id`,`sub`.`name` as `subject_name`,`sub`.`code` as `subject_code`,`subject_timetable`.*,class_sections.id as class_section_id, rooms.id as 'room_id', `rooms`.`room_name`, `rooms`.`room_code` FROM `subject_timetable` INNER JOIN `rooms` ON `rooms`.`id` = `subject_timetable`.`room_no`  INNER JOIN `classes` on classes.id = `subject_timetable`.`class_id` INNER JOIN sections on `sections`.`id`=`subject_timetable`.`section_id` INNER JOIN `subject_group_subjects` on `subject_group_subjects`.`id`=`subject_timetable`.`subject_group_subject_id` INNER JOIN `subjects` as `sub` on `sub`.`id`=`subject_group_subjects`.`subject_id`  INNER JOIN class_sections on class_sections.class_id=subject_timetable.class_id and class_sections.section_id=subject_timetable.section_id WHERE 1 and subject_timetable.session_id=" . $this->db->escape($this->current_session).$condition . " and subject_timetable.day=" . $this->db->escape($day_value)." and subject_timetable.staff_id=" . $this->db->escape($staff_id);
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
